...
\begin{itemize}
    \item \bf{function/map} from A to B : each element of A exactly $f$-related to one element of B.
        \begin{itemize}
            ...
            $ \forall x \in A \: \exists y_1,y_2 \in B \:\: ((x,y_1) \in f \land (x,y_2) \in f \then y_1 = y_2)$
...
\begin{itemize}
    ...
    \begin{itemize}
        \item All functions that have an inverse are bijective
...
\begin{center}
    Let $A$ be a set of size $n$. Number of subsets of $A$ of size $r$ is 
...
\begin{itemize}
    ...
    \item a \bf{proper} subgraph of $G$ is a subgraph $G$ of $G$ such that $H \neq G$
\end{itemize}
...
\begin{itemize}
    ...
    \end{enumerate}
    \item P11.3.6 - Let G be an undirected graph, then every vertex $v$ in $G$ is in some connected component of $G$
    \item T11.3.7 - Let $u,v$ be vertices in an undirected graph $G$. Then there is a path between $u$ and $v$ in G $\iff$ there is a connected component $G$ that has both $u$ and $v$ in it
\end{itemize}
