...
\begin{multicols}{3}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item - Any tree with $n$ vertices $(n>0)$ has $n-1$ edges.
        \item - If $G$ is any connected graph, $C$ is any circuit in $G$, and one of the edges of $C$ is removed from $G$, then the graph that remains is still connected.
        ...
        \item - For non-negative integers $h$, if $T$ is any binary tree with height $h$ and $t$ terminal vertices, then $t \leq 2^h$.
        \item - 
        \begin{enumerate}
